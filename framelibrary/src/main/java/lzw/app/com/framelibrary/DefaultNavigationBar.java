package lzw.app.com.framelibrary;
...
import lzw.app.com.baselibrary.navigationbar.AbsNavigationBar;
...
public class DefaultNavigationBar extends AbsNavigationBar<DefaultNavigationBar.Builder.DefaultNavigationParams> {
    ...
    public int bindLayoutId() {
        return lzw.app.com.framelibrary.R.layout.title_bar;
    }

    @Override
    public void applyView() {
        //绑定效果
        setText(lzw.app.com.framelibrary.R.id.center_title, getParams().mCenterTitle);
        if (!TextUtils.isEmpty(getParams().mRightText)) {
            setText(lzw.app.com.framelibrary.R.id.right_title, getParams().mRightText);
            setOnClickListener(lzw.app.com.framelibrary.R.id.right_title, getParams().mRightClickListener);
        }
        setText(lzw.app.com.framelibrary.R.id.left_title, getParams().mLeftText);
        if (!TextUtils.isEmpty(getParams().mLeftText)) {
            setVisible(lzw.app.com.framelibrary.R.id.left_image, View.GONE);
            setOnClickListener(lzw.app.com.framelibrary.R.id.left_title, getParams().mLeftClickListener);
        } else {
            setOnClickListener(lzw.app.com.framelibrary.R.id.left_image, getParams().mLeftClickListener);
        }
        if (getParams().mLeftIcon != 0) {
            setLeftIcon(lzw.app.com.framelibrary.R.id.left_image, getParams().mLeftIcon);
        }
